...
def png_rgb(width: int, height: int, pixels: bytes | bytearray) -> bytes:
    """Encode bounded top-to-bottom RGB8 diagnostics without another image library."""
    if (
        not image_dimensions_supported(width, height)
        or len(pixels) != width * height * 3
    ):
        raise ValueError("Invalid RGB image dimensions/buffer")
