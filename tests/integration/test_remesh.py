...
@pytest.mark.parametrize("ui,count", [(False, 25), (True, 27)])
def test_native_voxel_remesh(
    ...
    command = [
        ...
        "1",
        "--python",
        str(ROOT / "tests/blender/remesh_checks.py"),
    ]
    if ui:
    ...
    (tmp_path / "remesh-native.log").write_text(log)
    assert result.returncode == 0, log
    assert f"BLENDER_REMESH_TESTS_PASSED {count}" in log, log
